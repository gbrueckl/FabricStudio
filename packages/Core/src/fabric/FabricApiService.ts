...
import { GenericApiService } from '@utils/GenericApiService';
...
export abstract class FabricApiService {
	...
	static async initialize(
		...
		try {
			...
			this._apiBaseUrl = Helper.trimChar(CoreConfiguration.apiUrl, '/');
			this._tenantId = CoreConfiguration.tenantId;
			this._clientId = CoreConfiguration.clientId;
			this._authenticationProvider = CoreConfiguration.authenticationProvider;
			this._resourceId = CoreConfiguration.resourceId;
	...
	static async get<TSuccess = any>(
		...
		Core.Logger.logDebug("GET " + endpoint);
	...
	static async getList<TSuccess = any>(
		...
		let response = await this.get<iFabricListResponse<TSuccess>>(endpoint, params);

		if (response.error) {
			return { "error": response.error };
		}

		let ret: TSuccess[] = response.success[listProperty];

		while (response.success.continuationUri) {
			ret = ret.concat(response[listProperty]);
			response = await this.get<iFabricListResponse<TSuccess>>(response.success.continuationUri, undefined, { "raw": false });
	...
	static async post<TSuccess = any>(
		...
		Core.Logger.logDebug("POST " + endpoint + " --> " + (JSON.stringify(body) ?? "{}"));
