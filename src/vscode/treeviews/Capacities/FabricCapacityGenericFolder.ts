...
export class FabricCapacityGenericFolder extends FabricCapacityTreeItem {
	protected _customApiUrlPart: string;
	protected _defaultChildCollapsibleState: vscode.TreeItemCollapsibleState;
	protected _children: FabricCapacityTreeItem[] = undefined;
	...
	addChild(value: FabricCapacityTreeItem, parent: FabricCapacityTreeItem = this): void {
		...
		if (this._children) {
			...
			try {
				...
				for (let item of items.success) {
					let treeItem = new FabricCapacityTreeItem(item.id, item.displayName, item.type, this, item, item.description, this._defaultChildCollapsibleState);
