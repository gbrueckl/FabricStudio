...
export class FabricMirroredDatabaseSynchronization extends FabricWorkspaceTreeItem {
	constructor(
		parent: FabricWorkspaceTreeItem
	) {
		super(parent.id + "/Synchronization", "Synchronization", "MirroredDatabaseSynchronization", parent, undefined, undefined, vscode.TreeItemCollapsibleState.None);
	...
	async updateMirroringStatus(): Promise<void> {
		...
		if(currentStatus == "Stopped") {
			...
			actions.push("START_MIRRORING");
		}
		else if(currentStatus == "Running") {
			this.iconPath = new vscode.ThemeIcon("sync~spin");
			actions.push("STOP_MIRRORING");
		}
		else {
